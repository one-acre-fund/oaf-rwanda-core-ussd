...
module.exports = function(account_number, input_menu_name, an_table, lang, max_chars){ //here there be tygers
    ...
    for(prod in products){
        var prod_row = input_table.queryRows({'vars' : {'input_name' : prod}}).next();
        var input_name = prod_row.vars['input_name'].replace(/ /g,"_");
        if(parseFloat(client.vars[input_name]) > 0){
            currentPrice = parseFloat(client.vars[input_name])*parseFloat(prod_row.vars.price);
            totalPrice = totalPrice + currentPrice;
            tempstr = prod_row.vars[lang]+':'+client.vars[input_name]+' ' +prod_row.vars.unit+' - '+(parseFloat(client.vars[input_name])*parseFloat(prod_row.vars.price))+'RWF';
            outstr = outstr + '\n' + tempstr;
        }
    }
