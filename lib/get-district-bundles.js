...
function translate_input_names(input_name, lang){
    try {
        var inputTranslationsTable = project.getOrCreateDataTable('Input_Name_Translations');
        var cursor = inputTranslationsTable.queryRows({ "vars": { "input_name": input_name } });
        if(cursor.hasNext()){
            var row = cursor.next();
            if(row.vars[lang]){
...
function processBundles(districtBundles, districtId) {
    ...
    districtBundles.bundleInputs.forEach(function (bundleInput) {
        ...
        console.log("#### found bundleInput: " + bundleInput.InputName + " Price: "+ bundleInput.CostAdjustment_PerBundle);

        if (bundle) {
            
            const pb = {
                en: translate_input_names(bundleInput.InputName,'en'),
                increment: bundle.BundleQuantityIncrement,
                input_name: bundleInput.InputName,
                ki: translate_input_names(bundleInput.InputName,'ki'),
